...
//define(['text!./transition_save.xml', 'jsm-camelize'], function( htmlString, camelize) {
define(['text!./transition_save.xml'], function( htmlString) {	
	
	/**
	 * COMPONENT VIEWMODEL CONSTRUCTOR
	 */
	function transition_save( params) { 
//debugger;
		var _this = this;
...
// 		 * return d.promise();
// 		 * 
// 		 * Option 2: synchronous callback to cancel transition_save.
// 		 * 			 all subsequent lifecycle events will be cancelled and the state will remain unchanged.
// 		 * return false;
// 		 * 
// 		 * Option 3: synchronous callback to continue transition_save processing
// 		 * return true;
// 		 */
		var onBeforeClick = function(lifecycle) {
			...
			return true; // false - to cancel transition_save
		};
        ...
        this.click = function () {
			f.workflow.fsm[f.workflow.fsm.camelize(id)](f.state()); // trigger transition_save execution;
		};
    ...
    //componenttransition_save.prototype.doSomething = function() { ... };
	transition_save.prototype.schema = {
		"Params": {
			"namespace": "listItem",
			"visible": true,
    ...
    return { viewModel: transition_save, template: htmlString };
});


 //@ sourceURL=transition_save